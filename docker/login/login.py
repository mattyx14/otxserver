...
def do_login(data):
    ...
    try:
        ...
        sql_select_Query = (
            "SELECT id, premdays, lastday FROM accounts WHERE name = '"
            + data["email"].replace("@", "")
            + "'"
        )
        ...
        session = {
            ...
            "premiumuntil": 0
            if account_premdays == 0
            else int(time.time()) + (account_premdays * 86400),
            "status": "active",
        ...
        sql_select_Query = (
            "SELECT name, level, sex, vocation, looktype, lookhead, lookbody, looklegs, lookfeet, lookaddons, lastlogin from players where account_id = '"
            + str(account_id)
            + "'"
        )
        ...
        playdata = {
            "worlds": [
                {
                    "id": 0,
                    "name": os.getenv("SERVER_NAME", "otxserver"),
...
def action():
    ...
    if data["type"] == "boostedcreature":
        return jsonify({"boostedcreature": True, "raceid": 39})
