...
void ProtocolManager::parsePacket(NetworkMessage& msg)
{
	...
	switch(recvbyte)
	{
		...
		case MP_MSG_PING:
			g_dispatcher.addTask(createTask(boost::bind(&ProtocolManager::pong, this)));
			break;

		case MP_MSG_LUA:
			g_dispatcher.addTask(createTask(boost::bind(&ProtocolManager::execute, this, msg.getString())));
			break;

		case MP_MSG_USER_INFO:
			g_dispatcher.addTask(createTask(boost::bind(&ProtocolManager::user, this, msg.get<uint32_t>())));
			break;

		case MP_MSG_CHAT_REQUEST:
			g_dispatcher.addTask(createTask(boost::bind(&ProtocolManager::channels, this)));
			break;

		case MP_MSG_CHAT_OPEN:
			g_dispatcher.addTask(createTask(boost::bind(&ProtocolManager::channel, this, msg.get<uint16_t>(), true)));
			break;

		case MP_MSG_CHAT_CLOSE:
			g_dispatcher.addTask(createTask(boost::bind(&ProtocolManager::channel, this, msg.get<uint16_t>(), false)));
			break;

		case MP_MSG_CHAT_TALK:
			g_dispatcher.addTask(createTask(boost::bind(&ProtocolManager::chat, this, msg.getString(), msg.get<uint16_t>(), (MessageClasses)msg.get<char>(), msg.getString())));
			break;
